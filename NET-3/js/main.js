...
let myId = null, targetId = null;
let localCandidates = [];
let role = null; // 'receiver' 或 'sender'
let receiveBuffer = [], receivedFileName = '', receivedFileSize = 0;
let serverConnected = false;
let serverUrl = "192.168.43.15:8800";
let stunServer = "stun:stun.l.google.com:19302";
let peerNodes = [];
let publicIpInfo = null;
let manualPublicIpInfo = null;
let scanningActive = false;
let pendingIceCandidates = []; // 缓存早期ICE
...
const applyStunBtn = document.getElementById('applyStunBtn');
const publicIpPortInput = document.getElementById('publicIpPort');
const applyPublicBtn = document.getElementById('applyPublicBtn');
if (!applyPublicBtn) console.error('applyPublicBtn not found');
if (!publicIpPortInput) console.error('publicIpPortInput not found');
const logBox = document.getElementById('logBox');
const qrQuickPanel = document.getElementById('qrQuickPanel');
const genOfferSection = document.getElementById('genOfferSection');
const scanSection = document.getElementById('scanSection');
const radarView = document.getElementById('radarView');
const transferAssistant = document.getElementById('transferAssistant');
const myIdDisplay = document.getElementById('myIdDisplay');
const peersGroup = document.getElementById('peersGroup');
...
function handleSignalingMessage(msg) {
    ...
    if (msg.type === 'ok') {
        ...
        addLog(`[信令] 我的ID: ${myId}`);
        if (radarView.style.display === 'flex' && publicIpInfo) {
            ws.send(JSON.stringify({ type: 'report_public', payload: publicIpInfo }));
        }
...
applyPublicBtn.onclick = () => {
    console.log('applyPublicBtn clicked');
    if (!publicIpPortInput) {
        console.error('publicIpPortInput not found');
        return;
    }
    const value = publicIpPortInput.value.trim();
    if (value) {
        const lastColon = value.lastIndexOf(':');
        if (lastColon > 0 && lastColon < value.length - 1) {
            const ip = value.substring(0, lastColon);
            const portStr = value.substring(lastColon + 1);
            const port = parseInt(portStr);
            if (!isNaN(port) && port > 0 && port <= 65535) {
                manualPublicIpInfo = { public_ip: ip, public_port: port };
                publicIpInfo = manualPublicIpInfo; // 更新 publicIpInfo
                addLog(`手动公网IP/端口: ${ip}:${port}`);
                // 如果雷达模式正在运行且已连接信令，立即更新公网信息
                if (radarView.style.display === 'flex' && ws && ws.readyState === WebSocket.OPEN) {
                    ws.send(JSON.stringify({ type: 'report_public', payload: publicIpInfo }));
                    addLog('[雷达] 公网信息已更新');
                }
            } else {
                addLog('[错误] 端口号无效 (1-65535)');
            }
        } else {
            addLog('[错误] 格式应为 IP:端口');
        }
    } else {
        manualPublicIpInfo = null;
        publicIpInfo = null; // 清除 publicIpInfo
        addLog('已清除手动公网IP/端口');
    }
};
...
document.getElementById('radarCard').onclick = async () => {
    try {
        await startRadarMode(); // 在radar-detect.js中定义
    } catch(e) {
        alert('连接失败，请到高级功能调整信令服务器');
    }
};
